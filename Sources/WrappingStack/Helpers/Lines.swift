...
struct IndexedLines<S: RandomAccessCollection, Weight: AdditiveArithmetic & Comparable> {
    typealias Element = S.Element
    typealias Index = S.Index
    
    var elements: S
    var spacing: Weight
    var length: (Index, Element) -> Weight
    ...
    func split(lengthLimit: Weight) -> [Range<Index>] {
        ...
        for index in elements.indices {
            ...
            let elementLength = length(index, element)
            let newLength = currentLength + elementLength
